<template>
	<div>
	  ...
	  <div v-if="showModal" class="fixed inset-0 flex items-center justify-center z-50">
		<div class="bg-white rounded-lg p-6 shadow-lg md:w-1/3 sm:w-full bg-neutral_10">
		  ...
		  <p class="text-neutral_70 text-left text-sm font-normal">Apakah Anda yakin untuk menghapus item ini? Setelah dihapus item ini tidak bisa dikembalikan.</p>
		  <div class="mt-4 flex justify-end gap-3">
			...
			<!-- Delete button -->
			<button @click="deleteItem" class="text-neutral_10 rounded-lg bg-danger_main px-4 py-2">Hapus</button>
		  </div>
			</div>
		</div>
		</div>
